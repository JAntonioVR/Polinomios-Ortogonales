...
\begin{figure}[ht] 
    ...
    \cb{REVIEW Aquí tengo tela de ganas de decir que la cadena es irreducible, aperiódica (si algún $r_n > 0$) y aplicar el teorema \ref{th:teorema-estacionaria} y por tanto admite una única distribución límite, pero lo primero que la cadena no es finita, lo segundo que en ningún momento se específica que haya algún $r_n>0$ que la haga aperiódica.}

    \cb{TODO Referencia al libro y apartado que no recuerdo en el que se extendían los teoremas al infinito.}
    ...
    Recordamos algunos conceptos fundamentales de análisis funcional.
    ...
    \cb{REVIEW Esto de aquí arriba lo pongo como un lema o lo dejo así? No tengo la demostración ciertamente, pero no creo que merezca la pena quebrarse la cabeza con esto.}
    ...
    A partir de aquí, la idea es encontrar un operador lineal que induzca un producto escalar a partir del cual obtener una ortogonalidad para los polinomios.

    \section{La fórmula de representación de Karlin-McGregor}

    \cb{REVIEW Lidia qué opinas de que primero ponga la sección del teorema espectral y después defina los procesos de nacimiento y muerte? Es que así siento que es fácil descolgarse del tema de los procesos de nacimiento y muerte para ahora en esta sección recordarlos. Pero por otro lado soltar como primera sección el teorema espectral no sé si es buena idea.}
    ...
    Ahora si, por el teorema espectral, podemos afirmar que existe una medida $\psi$, cuyo soporte está contenido en $[-\|P\|,\|P\|]\subseteq[-1,1]$, tal que 
    \begin{equation}
        \int_{-1}^1 f(x)d\psi(x) = \prodesc{f(Pe^{0})}{}
    \end{equation}

    \cb{REVIEW No encuentro del todo la relación entre el teorema espectral y la existencia de esa norma.}
