...
\begin{proof}
    ...
    Por tanto $\int_a^b P'_n(x)x^{k-1}[\sigma(x)\rho(x)]dx=0$ para $k<n$, esto es, $\{P'_n\}$ es una SPO respecto a $\sigma(x)\rho(x)$.

    \cb{REVIEW Faltaría comprobar que $\int_a^b P'_n(x)x^{n-1}[\sigma(x)\rho(x)]dx\neq0$ no? }
\end{proof}

A partir de este último resultado podemos probar la consecuencia que ya adelantábamos: la sucesión de las derivadas también es clásica.
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    Nótese que $\tau_1(x)=\tau(x)+\sigma'(x)$ es de nuevo un polinomio de grado $1$.

    \cb{REVIEW Faltaría comprobar que cumple las condiciones de frontera, pero he leído en \cite[Página 90]{renato} que las condiciones de frontera se verifican incluso sin tener que resolver la ecuación de Pearson.}
\end{proof}
