...
\begin{lema}
    ...
    \end{enumerate}
    siempre que $\rho(x)$ verifique las condiciones de frontera 
    \begin{equation}
...
\begin{proof}
    \begin{itemize}
        \item[\fbox{$\Rightarrow$}] Queremos comprobar la igualdad de funcionales $D(\sigma \uu)=\tau\uu$. Para ello, tengamos en cuenta que dos funcionales lineales son iguales si, y solo si actuán igual sobre una base de $\mathbb P$. Escogemos la base $\{x^n\}_{n\geq 0}$. Entonces tenemos que
        $$
        ...
        \begin{equation*}
            ...
            \end{split}
        \end{equation*}
        \item[\fbox{$\Leftarrow$}] \cb{REVIEW El recíproco me está quitando años de vida.
        
        Queremos probar que la función peso $\rho(x)$ es solución de la ecuación \eqref{eq:Pearson-peso}, sabiendo que el funcional $\uu$ definido como en \eqref{eq:funcional-peso} verifica la ecuación de Pearson \eqref{eq:Pearson-u}.

        Por un lado, tenemos que

        $$
        \int_a^b \pi(x)\tau(x)\rho(x)dx = \prodesc{\tau \uu}{x^n}= \prodesc{D(\sigma \uu)}{x^n} = -\int_a^b \pi'(x) \sigma(x)\rho(x) dx.
        $$
        ...
        \begin{equation*}
            \begin{split}
                ...
                &= \left\{\begin{array}{ll}
                    ...
                    dv = \pi'(x) dx & v = \pi(x)
                \end{array}\right\} \\
                &= \int_a^b \pi(x)\tau(x)\rho(x)dx + \underbrace{\left[\sigma(x)\rho(x)x^n\right]_a^b}_{=0\text{ por (\ref{eq:cond-frontera})}} - \int_a^b \pi(x)[\sigma(x)\rho(x)]'dx \\
                &= \int_a^b \pi(x)(\tau(x)\rho(x) - [\sigma(x)\rho(x)]' )dx
            \end{split}
        \end{equation*}
        para cualquier polinomio $\pi(x)$.
...
Obsérvese que, a pesar de que $\sigma(x)$ puede estar definida en todo $\R$, tan sólo utilizamos valores de $x$ en los que $\sigma(x)\geq 0$. La justificación de esta elección se encontrará más adelante cuando resolvamos la ecuación de Pearson en cada caso, aunque adelantamos que los intervalos elegidos son los que hacen que la función peso $\rho$ sea positiva.
