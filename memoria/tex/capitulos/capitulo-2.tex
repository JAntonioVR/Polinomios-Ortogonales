...
\cb{REVIEW Nona me dice que ponga `\textbf{una} SPO', no `\textbf{la} SPO' para un funcional. Suelo asumir la unicidad salvo constante o salvo estandarización, de ahí que tenga una sección hablando de estandarizaciones. Aún así, ¿pongo mejor `una' para no asumir esa unicidad?}
...
\begin{proof}
    \begin{itemize}
        ...
        \cb{REVIEW ¿En realidad no sería que esa ecuación se verifica para casi todo punto? CONTINUIDAD}
    \end{itemize}
...
\cb{TODO Cambiar esto por una primitiva de $\tau/\sigma$}
...
Por último, consideremos $\sigma(x) = 1-x^2$ y $\tau(x)=Ax+B$. Ahora $\sigma$ tiene raíces simples en $x=-1,1$. Por este motivo, la solución $\rho$ de la ecuación de Pearson debe estar necesariamente definida en $(-\infty,-1)$, $(-1,1)$ ó $(1,+\infty)$.
\begin{equation*}
    \begin{split}
...
\cb{REVIEW Algo que no sea correcto?}
\begin{proposicion}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    Por (\cb{TODO Encontrar referencia del lema que no sea el TFG de un buen chaval}), tenemos que
    $$
...
\begin{proof}
    ...
    \end{itemize}
\end{proof}
\cb{REVIEW Corregido, pero un detalle. Resulta que, aunque tenemos que $1\Rightarrow 4$ porque el teorema \ref{th:ec-hipergeometrica} dice que si $\{P_n\}$ es clásica entonces es solución de la ecuación hipergeométrica, si nos fijamos en la demostración lo primero que hago es usar que las derivadas son ortogonales, por tanto, ¿podría decir que (4) es consecuencia de (3)? De poder hacerlo podría encadenar $1\Rightarrow 2\Rightarrow 3 \Rightarrow 4 \Rightarrow 5 \Rightarrow 1$.}
