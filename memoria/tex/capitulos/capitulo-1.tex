...
No obstante, en la mayoría de los casos y por simplicidad en lugar de utilizar una función $\psi$ y su medida inducida, si $\psi$ es absolutamente continua podemos reescribir (\ref{eq:prodescLpalpha}) como una integral de Lebesgue
...
Daremos ahora una nueva definición de SPO, en este caso con respecto a un funcional lineal.
...
Sea $\mathcal{L}$ un funcional lineal y $\{P_n\}$ una sucesión de polinomios. Las siguientes afirmaciones son equivalentes:
\begin{enumerate}
...
Por ejemplo, los polinomios de Tchebichef de tipo I, ortogonales en $[-1,1]$ suelen ser presentados verificando $T_n(1)=1$. Por su parte, los polinomios de Jacobi $P_n^{(\alpha,\beta)}$, que también son ortogonales en $[-1,1]$ (ver sección \ref{subsection:jacobi}), se busca que verifiquen $P_n^{(\alpha,\beta)}=\binom{n+\alpha}{n}$.
...
\begin{teorema}
    Sea $\{\mu_n\}$ una sucesión de números complejos y sea $\mathcal{L}$ un funcional lineal cuya sucesión de momentos es $\{\mu_n\}$. $\mathcal L$ admite una sucesión de polinomios ortogonales si, y solo si el determinante de momentos $\Delta_n\neq 0\ \ \ \forall n\in\N_0$. 
\end{teorema}
...
\begin{corolario}
    Un funcional de momentos $\mathcal{L}$ es definido positivo si, y solo si $\mathcal{L}[\pi^2(x)]>\nolinebreak0$ para cualquier polinomio $\pi(x)$.
\end{corolario}
...
Con las ecuaciones (\ref{eq:ctes-RRTT}) podemos construir la SPO utilizando la RRTT si conocemos el funcional, deduciendo simultáneamente los coeficientes de la RRTT y los polinomios que conforman la sucesión. Por su parte, utilizando (\ref{eq:ctes-RRTT2}) podemos calcular de una manera sencilla los coeficientes de la RRTT sin utilizar el funcional ni integrales, en ocasiones complejas. Sin embargo, en este último caso necesitamos conocer los polinomios $P_{n-1}, P_n$ y $P_{n+1}$ para obtener dichos coeficientes, no pudiendo construir la SPO como sí podíamos hacer si aplicábamos (\ref{eq:ctes-RRTT}). Comprobemos este hecho con el siguiente ejemplo.

\begin{ejemplo}
    \label{ej:hermite-rr3t}
    Vamos a clarificar estas ideas con un ejemplo práctico. En el archivo \texttt{software/Hermite.ipynb}\footnote{Puede ser consultado en la URL \url{https://github.com/JAntonioVR/Polinomios-Ortogonales/blob/main/software/Hermite.ipynb}} se ha implementado, utilizando el software matemático `\href{https://www.sagemath.org/}{SageMath}', el cálculo de una SPO mediante la RRTT. También se ha realizado una estimación de los tiempos de ejecución para distintos valores de $n$. En este ejemplo únicamente se recogen los resultados calculados por el software, pero en dicho archivo se puede consultar e incluso modificar el código fuente.
...
\begin{proof}
    ...
    Por último, véamos que $\mathcal{L}$ es definido positivo si y solo si $\gamma_n >0$ $\forall n\in \N_0$. Sea $\pi(x)$ un polinomio y $m$ su grado. Podemos entonces escribir $\pi(x)=\displaystyle\sum_{i=1}^m c_i P_i(x)$. De esta forma
    $$
    \mathcal{L}[\pi^2(x)]= \sum_{i=1}^m  c_i^2 \mathcal{L}[P_i^2(x)],
    $$
...
A partir de este concepto y los siguientes teoremas podremos conocer la dinámica que siguen los ceros de los polinomios ortogonales. En adelante, $\mathcal{L}$ es un funcional definido positivo y $\{P_n\}$ una SPO respecto a $\mathcal{L}$.
...
\begin{ejemplo}
    ...
    Además, el primer cero de cada polinomio es cada vez menor, de forma que el primer cero negro es el más pequeño. Análogamente, el último cero es cada vez mayor, siendo el negro el mayor de todos. Este hecho ejemplifica el corolario \ref{cor:sucesiones-ceros} con $k=1$.
