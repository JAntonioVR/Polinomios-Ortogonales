...
\begin{ejemplo}
    ...
    \begin{enumerate}
        ...
        \cb{REVIEW como alineo estas cosas sin meter 800 paquetes?}
...
Hemos clasificado los estados según su accesibilidad, pero fijémonos ahora en la posibilidad de revisitar o no un estado. En el ejemplo \ref{ej:diagramas}(1) es posible visitar cualquiera de los tres estados cada dos transiciones, mientras que en el ejemplo \ref{ej:diagramas}(2) es posible mantenerse estado $1$ o en el $2$ iteración tras iteración. Sobre esta noción insiste la siguiente definición.
...
Así, la cadena del ejemplo \ref{ej:diagramas}.1 es periódica con $d=2$. Sin embargo, no podemos plantearnos la periodicidad de la cadena del ejemplo \ref{ej:diagramas}.2, ya que ésta no es irreducible.
...
\cb{TODO Aclarar que denotamos con vectores las funciones masa de probabilidad y aclarar la abreviatura `fmp' para que al personal no le pase como a mí}
...
\begin{proposicion}
    ...
    \begin{align}
        \label{eq:condiciones-limite}
        \pi_j &= \displaystyle\sum_{i=1}^N \pi_i p_{ij},\ \forall j\in S \ \text{ (1)},& \displaystyle\sum_{j=1}^N \pi_j &=1\ \text{ (2)}
    \end{align}
\end{proposicion}
\begin{proof}
...
Como vemos, las distribuciones límite y las distribuciones estacionarias guardan ambas una estrecha relación con las ecuaciones de la proposición \ref{eq:condiciones-limite}, y vemos que también la tendrán la existencia y unicidad de una y otra.

\begin{proposicion}
    Una función masa de probabilidad dada por un vector $\pi^* =(\pi_1^*,\dots,\pi_N^*)^T$ es una distribución estacionaria si, y solo si verifica las ecuaciones \ref{eq:condiciones-limite}.
\end{proposicion}
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item[\fcolorbox{black}{white}{$\Leftarrow$}] Ahora asumimos que $\pi^*$ verifica las ecuaciones (\ref{eq:condiciones-limite}). Consideramos entonces que $P[X_0=j]=\pi_j^*, \ \ j\in S$. De nuevo, tomando $n=0$ en (\ref{eq:prob-total-cond}):
        $$
        P[X_{1}=j]=\sum_{i=1}^N P[X_0=i] p_{ij} = \sum_{i=1}^N \pi_i^* p_{ij} \underbrace{=}_{(\ref{eq:condiciones-limite}.1)}\pi_j^*.
        $$
        Por lo que la fmp de $X_1$ es $\pi^*$. Si usamos repetidamente (\ref{eq:prob-total-cond}) con distintos valores de $n$, deducimos que la fmp de $X_n$ es $\pi^*$ para cualquier $n\geq 0$. Esto es, $\pi^*$ es una distribución estacionaria.
    \end{itemize}
\end{proof}
